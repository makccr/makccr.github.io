...
However, after hooking everything up and compiling my 11ty site, I found that nothing appeared to have changed. I did a bit of research and found a great article form Martin Gunnarsson in which he [detailed himself going through the same process](https://www.martingunnarsson.com/posts/eleventy-syntax-highlighting/). He ran into problems when he didn't label code blocks through Markdown the was that PrismJS was expecting. If you are unfamiliar with Markdown, there are three different ways to denote code in Markdown:
...
It turns out that the syntax highlighting plugin from 11ty only supports the third option; a full code block with a label at the top line declaring what language the syntax highlighting needs to be applied to. Unfortunately for me, this is how I had already formatted damn near all of my code blocks site-wide. So I went ahead and took a closer look: 
...
But, before I brake down how I solved that problem, it is worth nothing that 11ty's plugin should automatically add sytax highlighting when it is installed. I have since confirmed that the only reason that this was not happening on my site is because I had previously added customization to the *code* and *pre* CSS classes, changing the text color. After removing that customization, the syntax highlighting worked as expected. But unfortunately, I didn't confirm that this was the case until after I had already spent nearly a full work day figuring out a way more complex work around, that just happens to add a higher level of customization.
...
And after setting up color definitions, I began the work of applying each color to the proper CSS classes. Luckily for me, 297 different languages only generated 34 different CSS classes that I needed to assign, but once again this process took a few hours of experimenting to get everything right. Eventually however I managed to get everything where it was meant to be, and ended up with what should be proper syntax highlighting for every single one of PrismJS's supported languages.
